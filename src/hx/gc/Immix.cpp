...
struct BlockDataInfo
{
         ...
         while(r<IMMIX_LINES)
         {
            if (rowMarked[r])
            {
               if (FULL)
               {
                  ...
                  if (starts)
                  {
                     unsigned int* headerPtr{ reinterpret_cast<unsigned int*>(mPtr->mRow + (r * IMMIX_LINE_LEN)) };
   ...
   {
      unsigned char time{ mPtr->mRow[inOffset + HX_ENDIAN_MARK_ID_BYTE_HEADER] };
      ...
      if (*reinterpret_cast<unsigned int *>(mPtr->mRow + inOffset) & IMMIX_ALLOC_IS_CONTAINER)
      {
         // See if object::new has been called, but not constructed yet ...
         void** vtable{ reinterpret_cast<void**>(mPtr->mRow + inOffset + sizeof(int)) };
         if (nullptr == vtable[0])
         {
      ...
      for(int h=0;h<mHoles;h++)
      {
         ...
         if (inOffset<last)
         {
            ...
            while(scan<=inOffset)
            {
               ...
               unsigned int header{ *reinterpret_cast<unsigned int*>(mPtr->mRow + scan) };
               unsigned int size{
                  !(header & 0xff000000)
                     ? (header & 0x0000ffff)
               ...
               if (inOffset>=scan && inOffset<end)
               {
                  ...
                  if (nullptr != outPtr)
                  {
                     *outPtr = reinterpret_cast<uintptr_t>(mPtr->mRow + scan + sizeof(int));
                  }

                  if (header & IMMIX_ALLOC_IS_CONTAINER)
                  {
                     // See if object::new has been called, but not constructed yet ...
                     void **vtable{ reinterpret_cast<void **>(mPtr->mRow + scan + sizeof(int)) };
                     if (vtable[0]==0)
                     {
                        // GCLOG("Partially constructed object.");
                        return allocString;
            ...
            if (r >= IMMIX_HEADER_LINES && r < IMMIX_LINES)
            {
               ...
               if ( allocStart[r] & rowPos )
               {
                  ...
                  if (result!=allocNone)
                  {
                     if (nullptr != outPtr)
                     {
                        *outPtr = reinterpret_cast<uintptr_t>(mPtr->mRow + blockOffset + sizeof(int));
                     }
   ...
   void VisitBlock(hx::VisitContext *inCtx)
   {
      ...
      for(int r=IMMIX_HEADER_LINES;r<IMMIX_LINES;r++)
      {
         if (rowMarked[r])
         {
            ...
            if (!starts)
               continue;
            unsigned char* row{ mPtr->mRow + (r * IMMIX_LINE_LEN) };
            for(int i=0;i<32;i++)
            {
               int pos = i<<2;
               if ( (starts & (1<<i)) &&
         ...
         for(int r=IMMIX_HEADER_LINES;r<IMMIX_LINES;r++)
         {
            if (rowMarked[r])
            {
               ...
               for(int i=0;i<32;i++)
               {
                  if ( starts & (1<<i))
                  {
                     unsigned int *row{ reinterpret_cast<unsigned int*>(from->mPtr->mRow + (r * IMMIX_LINE_LEN)) };
